/*
 * Форма телом ответа (gate.inline): страница берётся у auth-http внутренним
 * запросом, кладётся в обменник под ключ запроса и называется модулю секцией
 * rewrite реплая deny.
 *
 * Почему у сервиса, а не рендер здесь: форма -- это вёрстка источника, тексты,
 * провайдеры и их подсказки, и всё это живёт в auth-http. Второй рендерер в
 * инспекторе разошёлся бы с первым при первой же правке. Внутренний запрос
 * ходит только на показе формы, то есть на редком пути; горячий путь (allow)
 * в сеть не ходит.
 *
 * Билет выписывает инспектор -- тот же, что ставится клиенту кукой, -- и
 * передаёт его сервису кукой запроса: форма рендерится под этот же nonce, и
 * на POST входа он совпадёт с кукой клиента.
 */
...
const (
	...
	formSuffix = "form"
	// formTimeout -- бюджет внутреннего запроса: форма рендерится за
	// миллисекунды, а ждать дольше значит просрочить дедлайн волны.
	formTimeout = 2 * time.Second
)

var errNoHTTP = errors.New("WAF_AUTH_HTTP_URL is empty")
...
func newFormFetcher(base string) *formFetcher {
	return &formFetcher{
		client: &http.Client{
			Timeout: formTimeout,
			// Сервис отвечает формой, а не редиректом; редирект здесь --
...
/*
 * inlineForm -- форма входа для deny телом ответа. Возвращает секцию rewrite
 * либо ошибку: тогда вызывающий отвечает редиректом, как обычный профиль, --
 * билет у клиента уже есть.
 */
func (h *handler) inlineForm(ctx context.Context, req *protocol.Request,
	...
	if p.Src == nil {
		return nil, errors.New("profile without a source")
	}

	ticket := cookieOf(res.Cookies, p.Src.Ticket.Cookie)
	if ticket == "" {
	...
	body, ctype, err := h.forms.page(ctx, p.Src.Login.URI,
		p.Src.Ticket.Cookie+"="+ticket, req.Conn.ClientIP, userAgent)
	if err != nil {
		return nil, err
	}
...
// page -- GET формы от имени клиента: его адрес и User-Agent едут
// заголовками, как у прямого захода на форму.
func (f *formFetcher) page(ctx context.Context, path, cookie, clientIP,
	...
	if resp.StatusCode != http.StatusOK {
		return nil, "", fmt.Errorf("auth-http answered %d", resp.StatusCode)
	}
	...
	if len(body) > formMax {
		return nil, "", fmt.Errorf("form exceeds %d bytes", formMax)
	}

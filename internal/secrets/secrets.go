/*
 * Секреты профиля, приехавшего из контроллера.
 *
 * Профиль в манифесте открытых секретов не несёт -- ни пароля служебной записи
 * LDAP, ни секрета TOTP. Вместо них там ссылка store:<uuid>: сам секрет лежит в
 * Postgres конвертом, зашифрованным в браузере ключом контура, и контроллер его
 * не открывает.
 *
 * Открывает тот, у кого есть приватный ключ контура. Здесь это форма входа --
 * такой же держатель ключа, как агент на ноде nginx и crypto-сервис. Формат
 * конверта -- docs/config-distribution.md#формат-blob, четвёртая реализация
 * того же разбора; расхождение ловится тестом.
 *
 * Инспектор сюда не ходит: секреты нужны только при входе, а у него бюджет
...
const (
	// envelopeVersion -- единственная поддерживаемая версия конверта.
	envelopeVersion = 0x01
	nonceSize       = 12
	headerLen       = 1 + 2 // версия + длина обёрнутого DEK, big-endian uint16

	// Секрет читают на входе, а не на каждом запросе, поэтому кеш живёт долго:
	// смена секрета -- это новый uuid и новое поколение профиля, а не правка
	// содержимого по тому же адресу.
	cacheTTL = 15 * time.Minute
)

// Open разбирает конверт store-объекта приватным ключом контура.
...
// LoadKey читает приватный ключ контура из PEM.
...
/*
 * Store забирает конверт у контроллера и открывает его. Тем же путём и тем же
 * URL, что агент: GET /api/<scope>/store/<uuid>/blob.
 */
type Store struct {
	base   string
	scope  string
...
// New собирает клиента. Пустой base или отсутствующий ключ -- не ошибка: без
// них ссылки store: просто не открываются, и профиль, который их использует,
// сообщает об этом при входе.
func New(base, scope, keyFile string, timeout time.Duration) (*Store, error) {
	space := "default"

	/*
	 * Пространство можно назвать вместо uuid: uuid у каждой установки свой, и
	 * зашивать его в compose значило бы держать в конфиге значение, которое
	 * меняется при пересоздании базы.
	 */
...
// Get отдаёт открытый секрет по uuid store-объекта.
func (s *Store) Get(ctx context.Context, id string) (string, error) {
	if s == nil {
		return "", fmt.Errorf("store secrets are not configured: " +
			"set WAF_AUTH_CONTROLLER, WAF_AUTH_SCOPE and WAF_AUTH_CONTOUR_KEY")
	}
...
/*
 * resolveScope находит пространство по имени, если uuid не задан. Результат
 * запоминается: пространство не переименовывают на живом контуре, а лишний
 * запрос на каждый секрет незачем.
 */

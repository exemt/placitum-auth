 ...
 * ни имя, ни фаза.
 *
 * Записи пассивного отправителя не приезжают вовсе -- он не должен влиять на
 * трафик, в том числе нашими руками; что он сказал, видно в аудите.
 */
...
// Глаголы действий. Словарь общий для контура; из него калитка применяет
// только DoReauth и DoSkip -- остальные ей не адресованы, и загрузчик профиля
// отвергает правило с чужим глаголом.
const (
	...
	// Калитка его пока не шлёт: поля list у Action нет.
	DoBan = "ban"
)

// Оси: о ком высказывание. Reauth ездит только с осью session, skip -- только
// с request; модуль проверяет пару глагол-ось, несовместимая сюда не доедет.
const (
...
type Action struct {
	...
	// Delta -- только при DoThreshold, -1000..1000. Value -- при DoNote и
	// DoScore, со знаком. Калитка сама их не применяет, но и шлёт, и
	// разбирает: действие обязано доехать до аудита целиком.
...
type Request struct {
	...
	/*
	 * RequestStore -- объекты фазы запроса на фазах response и frame: там
	 * Store описывает объекты текущей фазы, а форму входа и куку запроса
	 * подсматривание читает из запроса. На фазе запроса секции нет.
	 */
 ...
 * каталога: так форма входа приходит на том же URI, без редиректа и без
...
/*
 * Session -- чью сессию калитка увидела на запросе: секция sessions реплая
 * (docs/verdict-protocol.md#секция-sessions). Модуль ничего из этого не
 * применяет, только переносит в запись аудита -- так журнал отвечает на
 * «кто это был» независимо от того, чья сессия: своя, чужой JWT или кука
 * приложения из списка доверенных.
 *
 * ID -- идентификатор, не сама кука: у своей сессии sid, у чужой -- хеш.
 * Сырой секрет на провод не кладут, запись живёт дольше запроса.
 */
type Session struct {
...
// Виды сессии в секции sessions: своя, чужой токен, кука приложения.
const (
	SessionOwn = "own"
	SessionJWT = "jwt"
	SessionApp = "app"
...
type Reply struct {
	...
	// Actions -- просьбы соседям и маршруту по правилам события профиля:
	// калитка знает о клиенте больше всех и с этого поколения говорит.
	Actions []Action `json:"actions,omitempty"`
	// Sessions -- от вердикта не зависит: allow с сессией и allow без неё --
	// разные записи журнала.

 ...
 * Admission control.
 *
 * Сообщение забираем с шины сразу: клиент NATS не очередь, pending — только
 * то, что уже в полёте к нам. Дальше решаем сами: прокисший бюджет выкидываем
 * из своего буфера (AUTH_DEADLINE_EXCEEDED), чтобы слот занял свежий запрос.
 * Полная очередь живых — drop (AUTH_QUEUE_LIMIT) или wait до места / дедлайна.
 * Молчание для модуля неотличимо от перегрузки.
 */
...
const (
	ReasonQueueLimit       = "AUTH_QUEUE_LIMIT"
	ReasonDeadlineExceeded = "AUTH_DEADLINE_EXCEEDED"

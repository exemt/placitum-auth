/*
 * Серверная сторона билета: одноразовость и счётчик попыток.
 *
 * Токен запечатан, и проверка подписи базы не требует; живость сессии держит
 * активный список (см. internal/livelist). Roster нужен ровно для того, чего
 * не умеют ни подпись, ни список:
 *
 *   nonce   -- форма принимает каждый билет один раз;
 *   попытки -- подбор пароля обязан упираться в блокировку.
 *
 * Отзыва здесь больше нет. Сессию завершает удаление записи из активного
 * списка -- механизм один: кука и список.
 */
...
// Burn -- одноразовость. true означает "этот вызов сжёг идентификатор первым";
// повтор того же nonce или того же кода TOTP получает false.
...
// LockedFor -- сколько ещё держать закрытым. Ноль означает "не заперт".
...
/*
 * Memory -- тот же интерфейс в пределах процесса. Годится для одной реплики и
 * для тестов; при scale > 1 nonce и попытки разъезжаются между репликами, и
 * это не деталь реализации, а свойство режима.
 */
type Memory struct {
	mu     sync.Mutex

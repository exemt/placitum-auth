...
/*
 * Put кладёт объект под ключ запроса: форму входа для deny телом ответа
...
// Args -- строка запроса как пришла; пусто -- её нет или не достать.
func Args(ctx context.Context, s *Redis, loc *protocol.Locator) string {
	return argsOf(ctx, s, loc)
}

// Объект обменника зашифрован, а ключа у калитки нет: подглядывание входа
// читает тело формы только открытым.
const UnavailableEncrypted = "store_encrypted"

/*
 * Body достаёт тело по локатору. Нужно одному месту -- подглядыванию входа
 * приложения на фазе ответа, и только на маршруте входа. Второе значение --
 * причина, по которой тела нет; пустая при nil-срезе означает, что маршрут
 * его не снимает.
 */
func Body(ctx context.Context, s *Redis, loc *protocol.Locator) ([]byte, string) {
	switch {
	case loc == nil:
		return nil, ""

	case loc.Unavailable != "":
		return nil, loc.Unavailable

	case loc.Driver == "":
		return nil, ""

	case loc.Enc != nil:
		return nil, UnavailableEncrypted
	}

	if s == nil {
		return nil, UnavailableNoStore
	}
	...
	switch {
	case errors.Is(err, redis.Nil):
		return nil, UnavailableNotFound

	case errors.Is(err, ErrUnknownDriver):
		return nil, UnavailableDriver

/*
 * LDAP и NTLM: пароль проверяет каталог, а не контур.
 *
 * Оба провайдера ходят к одному и тому же контроллеру домена и различаются
 * только тем, как доказывают знание пароля: простой bind отдаёт его каталогу,
 * NTLM -- проводит рукопожатие, при котором пароль по проводу не идёт.
 *
 * Ни тот ни другой не является HTTP-схемой WWW-Authenticate: логин и пароль
 * пользователь вводит в форму, а сервис проверяет их у домена. Прозрачный
 * доменный вход -- это состояние на соединение, которого у инспектора по
 * контракту нет; см. docs/README.md#ntlm.
 */
...
func (l *LDAP) Verify(ctx context.Context, c Credentials, id *Identity) (*Identity, error) {
	...
	/*
	 * Пустой пароль отбрасывается до похода в каталог. LDAP трактует bind с
	 * пустым паролем как анонимный и отвечает успехом -- это классическая
	 * дыра, и закрывается она здесь, а не настройками сервера.
	 */
	if login == "" || c.Password == "" {
	...
	/*
	 * Членство запрашивается после проверки пароля: неверному входу лишний
	 * поиск в каталоге не положен. Связь к этому моменту уже под пользователем,
	 * а читать чужие ветки он обычно не вправе -- отсюда вторая связь.
	 */
...
func (n *NTLM) Verify(ctx context.Context, c Credentials, id *Identity) (*Identity, error) {
	...
	/*
	 * Домен допускается в самом логине -- CORP\ivanov: так его вводят
	 * привычно, и заставлять пользователя стирать префикс ради поля формы
	 * означало бы ловить ошибку входа там, где её нет.
	 */
	domain := n.Cfg.Domain
	...
	/*
	 * Группы читаются отдельным поиском от служебной учётной записи: NTLM-bind
	 * прошёл на соединении, но искать от пользователя мы не можем -- пароль
	 * ему больше не понадобится, а второй bind разорвал бы уже доказанное.
	 */
...
/*
 * servicePassword достаёт пароль служебной записи. Две дороги и ровно одна на
 * профиль: переменная окружения у профиля из файла, store-объект у профиля из
 * контроллера. Какая именно -- решено при загрузке профиля, здесь только
 * исполнение.
 */
...
func tlsConfig(url string, cfg config.LDAPTLS) (*tls.Config, error) {
	out := &tls.Config{
		...
		InsecureSkipVerify: cfg.InsecureSkipVerify, //nolint:gosec // осознанная опция профиля
...
// lookup ищет запись пользователя от служебной учётной записи. Пароль этой
// записи -- из переменной окружения: профиль лежит в git и уезжает в KV.
func lookup(conn *ldap.Conn, s config.LDAPSearch, login, password string) (*ldap.Entry, error) {
	...
	/*
	 * Ровно одна запись. Ноль -- нет такого пользователя, две и больше --
	 * фильтр, который вошёл бы не за того: и то и другое отказ, а не выбор
	 * первой попавшейся.
	 */
...
// serviceBind входит служебной записью. Её пароль -- из переменной окружения:
// профиль лежит в git и уезжает в KV, пароль там не место.
func serviceBind(conn *ldap.Conn, s config.LDAPSearch, password string) error {
	...
	if err := ldapErr(conn.Bind(s.BindDN, password)); err != nil {
		// Служебная запись не вошла -- это авария контура, а не неудача
		// пользователя: в счётчик попыток такое не идёт.
...
/*
 * memberOf есть у Active Directory и у OpenLDAP с overlay memberof; без
 * overlay атрибута нет вовсе, и членство приходится спрашивать с другой
 * стороны -- см. reverseGroups.
 */
...
/*
 * reverseGroups ищет группы, в которых пользователь состоит, обратным
 * фильтром по member=<dn>. Это единственный способ узнать членство в каталоге
 * без overlay memberof, а таких инсталляций OpenLDAP большинство.
 *
 * Поиск идёт от служебной записи: связь пользователя к этому моменту уже
 * доказала пароль, но читать чужие ветки он обычно не вправе.
 */
...
/*
 * shortNames оставляет от DN группы только CN. Полный DN в заголовке
 * приложению -- это строка на двести байт, из которой приложение всё равно
 * вырезает первый компонент.
 */
...
/*
 * ldapErr переводит ответ каталога в одну из трёх наших ошибок. Разница между
 * "пароль не тот" и "каталог недоступен" существенна: первое идёт в счётчик
 * попыток, второе -- в лог ошибок и в пульс.
 */
